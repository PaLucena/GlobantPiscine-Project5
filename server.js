...
const	server = http.createServer(async (req, res) => {
	...
	if (req.url === '/') {
		fs.readFile(path.join(__dirname, 'unsplash-app/login/login.html'), (err, content) => {
			if (err) {
		...
		fs.readFile(path.join(__dirname, 'unsplash-app/login/loginStyles.css'), (err, content) => {
			if (err) {
		...
		fs.readFile(path.join(__dirname, 'unsplash-app/login/loginScript.js'), (err, content) => {
			if (err) {
				res.writeHead(404, { 'Content-Type': 'text/plain' });
